...
- 产出：`<课件名>_详细讲义.html`（中间产物，浏览器可直接看）+ `.pdf`（唯一交付版本，全图文、公式为 LaTeX 渲染、文字已轮廓化）。
...
```bash
python3 gen_<课件名>_pdf.py     # 打印 HTML 体积、PDF 体积与页数、轮廓化后的剩余字体数
```
...
根治办法是**把文字转成矢量轮廓**（页面里不再有任何字体，也就无从误解码）。`noteskit.build()` 默认 `flatten=True`：PDF 一写出来就地轮廓化，**只产出这一个版本**，构建日志会打印"剩余字体数 0，应为 0"。

- **代价：文字不能选中 / 搜索 / 复制**（只剩线条，没有字符信息）。确实需要可搜索版本时（例如自己要全文检索）才用 `nk.build(..., flatten=False)`，并在交付说明里讲清。
- 单独处理已存在的 PDF：`python3 scripts/flatten_pdf.py <输入.pdf> <输出.pdf>`，打印的"剩余字体数"必须为 0。
- 轮廓化后照旧用 `preview_pdf.py` 抽查渲染（封面 / 公式页 / 末页）。
...
- PDF/docx 是自包含容器，不依赖外部文件，但**内嵌字体可能被个别手机查看器误读**——交付的 PDF 默认已轮廓化（第 5 步），一般不会遇到；若某个旧文件在别人手机上乱码（字母整体错位、中文空白），不要怀疑内容，用 `flatten_pdf.py` 补一次轮廓化即可。自己在 MuPDF/Chromium 里看到的正常不代表对方正常。
...
- [ ] PDF 已轮廓化（构建日志"剩余字体数 0"），且抽查过渲染
- [ ] 文件命名：`<课件名>_详细讲义.pdf`（唯一交付版本，轮廓化，到处能开但不可搜索——交付时说明这一点），放课件同目录或用户指定位置

## 完整示例

`examples/` 里有一个从零跑通的完整示例：`示例课件.pptx`（5 页）→ `gen_示例课件_pdf.py` → `示例_详细讲义.pdf`（一份交付版本，已轮廓化），包含封面、目录、逐页原图对照、公式渲染、推导块与例题的全部写法，可作为新讲义的起手模板。
