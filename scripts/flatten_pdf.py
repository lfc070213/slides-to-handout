...
这类阅读器也无从出错；代价是文字不可选中/搜索（需要搜索时用 noteskit.build(..., flatten=False)）。

用法: flatten_pdf.py <输入.pdf> <输出.pdf> [起始页] [结束页]
注：noteskit.build() 默认会在写出 PDF 后自动调用这里，一般不用手动跑。
"""
import sys
import os
...
def count_fonts(pdf_path):
    """数 PDF 里还剩几个字体对象；轮廓化干净的文件应为 0。"""
